...
cat(model_tables$industrial_bic) # Use cat() to explicitly print the string
cat(model_tables$industrial_aic) # Use cat() to explicitly print the string
```
...
# Convert BIC table to LaTeX and print
cat(knitr::kable(model_tables$core_infl_bic, format = "latex", booktabs = TRUE, caption = "Core Inflation BIC Table"))
cat("\n\n") # Add some vertical space between the tables

# Convert AIC table to LaTeX and print
cat(knitr::kable(model_tables$core_infl_aic, format = "latex", booktabs = TRUE, caption = "Core Inflation AIC Table"))
...
# Convert BIC table to LaTeX and print
cat(knitr::kable(model_tables$credit_spread_bic, format = "latex", booktabs = TRUE, caption = "Credit Spread BIC Table"))
cat("\n\n") # Add some vertical space between the tables

# Convert AIC table to LaTeX and print
cat(knitr::kable(model_tables$credit_spread_aic, format = "latex", booktabs = TRUE, caption = "Credit Spread AIC Table"))
...
- **ARCH Test for Conditional Heteroskedasticity**  
  The null of no ARCH effects is *strongly rejected* (p ≈ 0), indicating time-varying volatility in the residuals. While this does not bias coefficient estimates, it does invalidate inference based on homoskedastic standard errors. This is why we will rely on *bootstrapped IRFs* in the next step.
...
```{r exercise2/5, echo=FALSE, fig.show='hold', fig.height=8, fig.width=8}
...
Industrial production (IP) growth responds notably: it falls sharply over the first two months (peak drop ≈ –0.3 SD), then slowly recovers, returning close to baseline around month 6–8.

Inflation (CPI) growth also dips initially (trough ≈ –0.25 SD at month 2) but recovers more quickly—already by month 3, effects are minor and mostly indistinguishable from zero.
...
*NB: Since all series are non-stationary, we had to apply first differencing before estimating the VAR. While this ensures statistical validity, it also makes the results somewhat harder to interpret economically, as we are now modeling changes rather than levels. As a result, the dynamics may appear less intuitive or economically rich. In such settings, approaches based on Vector Error Correction Models (VECMs) could offer more informative insights by explicitly capturing long-run relationships between the variables.*

## Exercise 2.6: Modify the ordering of the variables and comment how this affects your results

```{r exercise_2/6, echo=FALSE, fig.show='hold', fig.height=8, fig.width=8}
# reversed ordering
