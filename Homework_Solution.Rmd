...
# Create In-sample fit plot
plot(cbind(ipi_s1, ar3_s1$fitted), plot.type = "s", col = c("black", "red"), lty = 1,
     main = "AR(3) In-Sample Fit – Δlog Industrial Production",
...
# Out of sample plot 
plot(ar3_s1_fc, main = "AR(3) Out-of-Sample Forecast – Δlog IPI",
     ylab = "Δlog IPI", xlab = "Time")


```

*Comment on the Results* \* We will no plot the in-sample and out-of sample forecasts for the Industrial Production time series, remember initially the best performing model was an MA(3) process, however MA processes are generally worse than AR(p) processes for forecasting (add a reason here) \* So to begin this exercise we will check if the AR(3) process is also suited for modelling here given the residual diagnostics, we see that we pass the Ljung Box test and fail the Jarque Bera Test --\> again this is good enough for us to go also from AIC and BIC table we know the AR(3) process performs only slightly worse on the BIC than the MA(3) process
