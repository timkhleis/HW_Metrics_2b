...
## 2. Apply the Classical Manual Search process for optimal ARMA(p,q) Model---- 

# 1) Gahter all 3 series in a named list 
ts_list <- list(
...
# 2) Function: grid‐search (manually) and  extract top‐5 Models for both ICs
...
# 3) Apply to each series, fit the BIC-winner
...
# 4) Print top‐5 tables
for (nm in names(all_models)) {
  cat("\n=== Series:", nm, "===\n")
  cat("Top 5 by BIC:\n")
  print(all_models[[nm]]$topBIC)
  cat("\nTop 5 by AIC:\n")
  print(all_models[[nm]]$topAIC)
}

# 5) Plot each in‐sample fit (overlayed)
for (nm in names(all_models)) {
...
auto_models <- lapply(ts_list, function(y) {
  # force d=0 (we already differenced), no seasonality
  auto_fit <- auto.arima(y, d = 0, seasonal = FALSE, ic = "BIC",
